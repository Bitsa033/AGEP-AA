...
// $query="SELECT * FROM `eleve` WHERE nomprenom LIKE '{$clef}%' OR classe LIKE '{$clef}%'";
 ...
 // ".$_SESSION['l'].",3";
 $query="SELECT student.id as id,student.name as name,student.surname as surname,student.gender as gender,payment_of_school.price as price,payment_of_school.rest as reste,add_student.classroom as classroom,payment_of_school.resolve as resolve FROM `student`,add_student,payment_of_school WHERE student.id=add_student.student AND student.id=payment_of_school.student ORDER BY id ASC LIMIT
 ".$_SESSION['l'].",3";
  ...
  if ($exec->num_rows > 0) {
   ...
   <tr>
      ...
      <td>
        ...
        <h5> Scolarisé: <strong><?php echo $avc." FCFA "?></strong> Reste: <strong><?php echo $ir." FCFA "; ?></strong></h5>
      </td>
